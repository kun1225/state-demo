...
listener.startListening({
  ...
  effect: async (_action, api) => {
    const state = api.getState() as { preferences: { q: string; limit: number } };
...
const persistPreferences: Parameters<typeof configureStore>[0]["middleware"] = (getDefault) =>
  getDefault().concat(listener.middleware, (storeApi) => (next) => (action: any) => {
    ...
    if (
      ...
      const { preferences } = storeApi.getState() as { preferences: { q: string; limit: number } };
      try {
